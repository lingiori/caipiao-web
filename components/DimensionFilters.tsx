...
/** 数字筛选字段 key */
type NumberFilterField = "max_step" | "current_distance" | "tuijian_num";
...
/**
 * 维度信息筛选面板。
 * 提供维度名称模糊搜索与数字列（大于/等于/小于/区间）筛选。
 * 输入或下拉选择不会立即触发筛选，需按回车或点击“提交筛选”按钮才生效。
 */
export default function DimensionFilters({
  ...
  function handleValueChange(field: NumberFilterField, rawValue: string) {
    ...
    setDraft((prev) =>
      updateNumberFilter(prev, field, {
        value: rawValue === "" || isNaN(value ?? NaN) ? null : value,
      })
    );
  }
  ...
  function handleRangeChange(
    ...
    setDraft((prev) =>
      updateNumberFilter(prev, field, {
        [boundary]: rawValue === "" || isNaN(value ?? NaN) ? null : value,
      })
    );
  }
